...
function CountUp({ end, suffix }: { end: number; suffix: string }) {
	...
	useEffect(() => {
		...
		const controls = animate(0, end, {
			...
			onUpdate(value) {
				...
				const formatted =
					end % 1 !== 0 ? value.toFixed(1) : Math.floor(value).toLocaleString();
	...
	return (
		<span ref={ref} className="text-primary-foreground text-4xl font-bold">
			0{suffix}
		</span>
	);
}
	...
	return (
		<section className="bg-primary py-16">
			<div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
				<p className="text-primary-foreground/80 mb-10 text-center text-lg font-medium">
